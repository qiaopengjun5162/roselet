...
- 当前小目标：
  - [ ] 明确 Cloudflare 方案边界：Web 先上，还是全量迁移到 Workers
...
1. 写清楚 Cloudflare 路线：确认“能不能全上、哪些需要改、最稳妥的第一步是什么”。
2. 完成第一版上线方案文档：让项目当前所处阶段、架构边界、上线步骤对内对外都能讲明白。
3. 选定一条最小上线路径并执行：优先让 Web 可外网访问，再决定后端是否继续保留 Rust 独立部署。

## 下一步

- 我们下一步应该先做：`Cloudflare 部署方案文档 + 分阶段落地计划`
- 目的不是立刻改一堆代码，而是先把：
  - 最终目标
  - 当前阶段
  - 下一阶段的小目标
