...
- 当前阶段：`Beta 准备上线（Web 开始消费 Cloudflare Worker）`
...
1. 继续扩大前端切流范围：在只读接口之后决定下一个优先 API。
2. 完成第一版线上后端切换准备：确认生产 Worker 域名与 Vercel 环境变量。
3. 选定下一条最低风险迁移线：写接口优先，还是剩余认证接口优先。

## 下一步

- 我们下一步应该先做：`继续迁认证入口，准备让 Web 逐步切到 Worker`
- 当前已经完成：
  - `Vercel` 前端真实上线
