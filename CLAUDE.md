...
| `aleo.rs` | Aleo 私密记录的 domain-separated commitments | `build_aleo_vault_inputs_wasm` |
...
- **生产前端入口**：
  - 主入口：`https://roselet-web.vercel.app`（Vercel）
...
- **GOAT SDK 状态**：官方 `goat-sdk/goat` 仓库当前已归档；不要把它引入核心运行时。Agent 集成使用仍维护的 x402 Foundation HTTP/MCP 包，GOAT 类客户端通过标准接口调用。
...
PUT    /api/rose/:id       # 更新玫瑰设置（仅 owner；允许公开转私密，或给公开且未送礼的玫瑰补送）
