...
### 38. 链上交易哈希不是业务验证，必须校验回执事件

问题：前端可以提交任意一个看似合法的交易哈希；仅保存哈希不能证明它对应当前业务对象、当前钱包或当前消息。

根因：EVM 交易哈希只定位一笔交易，不带应用层的所有权语义。若后端不读取回执事件，用户可以复用旧交易或提交别的合约交易。

解决：后端通过受限链 RPC 读取 `eth_chainId` 和 `eth_getTransactionReceipt`，只接受成功交易中同时匹配合约地址、业务 id、钱包地址、消息哈希和业务枚举值的事件。数据库对业务 id 和 tx hash 都加唯一约束；用户钱包仍在浏览器内签名，服务端不保存用户私钥。

验证：单元测试锁定 bytes32 编码、地址 topic 匹配和哈希规范；集成测试锁定未验证记录的空响应；真实 testnet 交易上线前必须额外做一次端到端冒烟。

通用规则：把链上事件当作跨系统契约版本化维护。修改事件参数、索引字段或编码规则前，必须同时更新前端调用、后端回执校验、迁移和测试。
